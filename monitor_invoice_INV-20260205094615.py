...
TEMPLATE_NAME = "尺寸/孔位类"
CHECK_INTERVAL = 5  # 检查间隔（秒）
MAX_CHECKS = 120  # 最大检查次数（10分钟）

def find_invoice_and_template():
    """查找发票和模板信息"""
    with Session(engine) as session:
        ...
        if template_prompt:
            print(f"   ✅ 提示词已设置，长度: {len(str(template_prompt))} 字符")
            print(f"   提示词预览（前100字符）: {str(template_prompt)[:100]}...")
        else:
            print(f"   ⚠️  提示词未设置")
        
        # 检查 JsonSchema
        template_schema = getattr(template, 'schema', None)
        if template_schema:
            import json
            schema_str = json.dumps(template_schema, ensure_ascii=False) if isinstance(template_schema, dict) else str(template_schema)
            print(f"   ✅ JsonSchema 已设置，长度: {len(schema_str)} 字符")
            print(f"   JsonSchema 预览（前100字符）: {schema_str[:100]}...")
        else:
            print(f"   ⚠️  JsonSchema 未设置")
                        ...
                        class SimpleTask:
                            def __init__(self, **kwargs):
                                for k, v in kwargs.items():
                                    setattr(self, k, v)
                        tasks.append(SimpleTask(**task_dict))
    ...
    while check_count < MAX_CHECKS:
        ...
        with Session(engine) as session:
            ...
            if task.start_time:
                ...
                # 如果还没有启动，计算从创建到现在的等待时间
                pending_duration = (current_time - task.create_time).total_seconds()
            
            # 诊断 pending 状态卡在哪一步
            def diagnose_pending_status(task_obj, db_session):
                """诊断 pending 状态卡在哪一步"""
                steps = []
            ...
            if status != last_status or check_count == 1:
                ...
                if status == "pending":
                    ...
                    diagnosis = diagnose_pending_status(task, session)
            ...
            # 显示当前状态（pending 状态时每5次检查显示一次详细诊断）
...
def main():
    ...
    # 查找发票和模板
    invoice, template = find_invoice_and_template()
    ...
    if template:
        ...
        if not template_prompt:
            print("⚠️  警告: 模板提示词未设置，识别时可能无法使用 Input_Pro 参数")
        if not template_schema:
            print("⚠️  警告: 模板 JsonSchema 未设置，识别时可能无法使用 JsonSchema 参数")
