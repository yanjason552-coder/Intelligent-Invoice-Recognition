...
echo "创建部署准备脚本..."
cat > "$APP_DIR/scripts/aliyun/prepare-deployment.sh" << 'PREPARE_EOF'
#!/bin/bash
# 部署准备脚本
# 在服务器上运行，准备部署所需的所有配置
...
if [ ! -f "$APP_DIR/.env" ]; then
    if [ -f "$APP_DIR/scripts/aliyun/.env.production.template" ]; then
        ...
        echo -e "${YELLOW}⚠ .env 文件不存在，创建基本模板...${NC}"
...
chmod +x "$APP_DIR/scripts/aliyun/prepare-deployment.sh"
echo "✓ 部署准备脚本已创建"

echo ""
echo "现在可以运行："
echo "  bash $APP_DIR/scripts/aliyun/prepare-deployment.sh"
