...
class Invoice(SQLModel, table=True):
    ...
    # 模板版本（用于“不同模板字段不一致”的动态展示与审核）
    # 注意：该字段由 alembic 迁移 add_template_management_tables.py 添加到 invoice 表
    template_version_id: Optional[UUID] = Field(
        default=None,
        foreign_key="template_version.id",
        description="模板版本ID（用于动态字段展示）",
    )
...
class InvoiceResponse(SQLModel):
    ...
    latest_recognition_result_id: Optional[UUID] = None
...
class RecognitionTaskParams(SQLModel):
    ...
    template_id: Optional[UUID] = Field(default=None, description="模板ID（template_strategy=fixed 时使用）")
