...
# Debug mode log path (repo root/.cursor/debug.log)
def _get_repo_root() -> Path:
    p = Path(__file__).resolve()
    for parent in p.parents:
        if parent.name.lower() == "backend":
            return parent.parent
    # fallback: assume .../invoicepdf/backend/app/api/routes/invoice.py
    return p.parents[4]

DEBUG_LOG_PATH = _get_repo_root() / ".cursor" / "debug.log"
...
def upload_invoice(
    *,
    ...
    try:
        _dbg_log("pre-run", "H1", "invoice.py:upload_invoice", "enter upload_invoice", {
        ...
        if existing_file:
            ...
            allow_dup = (
                allow_duplicate
                and bool(getattr(current_user, "is_superuser", False))
                and settings.INVOICE_DEBUG_ALLOW_DUPLICATE_UPLOADS
            )
            if allow_dup:
                _dbg_log("pre-run", "H13", "invoice.py:upload_invoice", "duplicate allowed (debug)", {
                    "fileName": file.filename,
                    "existingFileId": str(existing_file.id),
                    "uploaderId": str(current_user.id),
                })
            else:
        ...
        logger.info("=== 票据上传成功 ===")
        _dbg_log("pre-run", "H1", "invoice.py:upload_invoice", "upload success", {
            "fileId": str(invoice_file.id),
        ...
        _dbg_log("pre-run", "H1", "invoice.py:upload_invoice", "http exception", {
            "fileName": file.filename if file else None
        })
        raise
    except Exception as e:
        logger.error(f"上传失败异常: {str(e)}", exc_info=True)
        _dbg_log("pre-run", "H1", "invoice.py:upload_invoice", "exception", {
            "fileName": file.filename if file else None,
            "error": str(e)
        })
...
def upload_invoice_external(
    *,
    ...
    try:
        _dbg_log("pre-run", "H2", "invoice.py:upload_invoice_external", "enter upload_invoice_external", {
            "fileName": file.filename if file else None,
            "contentType": file.content_type if file else None,
        ...
        if existing_file:
            ...
            if allow_dup:
                _dbg_log("pre-run", "H13", "invoice.py:upload_invoice_external", "duplicate allowed (debug)", {
                    "fileName": file.filename,
                    "existingFileId": str(existing_file.id),
                    "externalFileId": external_file_id,
                    "uploaderId": str(current_user.id),
        ...
        _dbg_log("pre-run", "H2", "invoice.py:upload_invoice_external", "upload external success", {
            "fileId": str(invoice_file.id),
            "invoiceId": str(invoice.id),
            "invoiceNo": invoice.invoice_no,
            "externalFileId": external_file_id
        ...
        logger.error(f"HTTP异常: {file.filename if file else 'unknown'}")
        _dbg_log("pre-run", "H2", "invoice.py:upload_invoice_external", "http exception", {
            "fileName": file.filename if file else None,
            "externalFileId": external_file_id
        })
        raise
        ...
        logger.error(f"上传失败异常: {str(e)}", exc_info=True)
        _dbg_log("pre-run", "H2", "invoice.py:upload_invoice_external", "exception", {
            "fileName": file.filename if file else None,
            "externalFileId": external_file_id,
            "error": str(e)
        })
...
def create_recognition_task(
    *,
    ...
    """
    try:
        _dbg_log("pre-run", "H3", "invoice.py:create_recognition_task", "enter create_recognition_task", {
            "invoiceId": str(task_in.invoice_id),
            "modelConfigId": str(task_in.params.model_config_id) if task_in.params and task_in.params.model_config_id else None,
            "recognitionMode": task_in.params.recognition_mode if task_in.params else None
        })
        # 验证票据是否存在
        ...
        _dbg_log("pre-run", "H3", "invoice.py:create_recognition_task", "exception", {
            "invoiceId": str(task_in.invoice_id),
            "error": str(e)
        })
...
def start_recognition(
    *,
    ...
    """
    try:
        _dbg_log("pre-run", "H4", "invoice.py:start_recognition", "enter start_recognition", {
            "taskId": str(task_id)
        })
        ...
        if task.status != "pending":
            _dbg_log("pre-run", "H8", "invoice.py:start_recognition", "reject start due to status", {
                "taskId": str(task_id),
                "status": task.status
            })
        ...
        _dbg_log("pre-run", "H4", "invoice.py:start_recognition", "exception", {
            "taskId": str(task_id),
            "error": str(e)
        })
...
def query_invoices(
    *,
    ...
    try:
        _dbg_log("pre-run", "H5", "invoice.py:query_invoices", "enter query_invoices", {
            "skip": skip,
            "limit": limit,
...
def approve_invoice(
    *,
    ...
    """
    try:
        _dbg_log("pre-run", "H6", "invoice.py:approve_invoice", "enter approve_invoice", {
            "invoiceId": str(invoice_id),
            "reviewerId": str(current_user.id) if current_user else None
        })
        invoice = session.get(Invoice, invoice_id)
        if not invoice:
            raise HTTPException(status_code=404, detail="票据不存在")
        ...
        _dbg_log("pre-run", "H6", "invoice.py:approve_invoice", "exception", {
            "invoiceId": str(invoice_id),
            "error": str(e)
        })
...
def reject_invoice(
    *,
    ...
    """
    try:
        _dbg_log("pre-run", "H6", "invoice.py:reject_invoice", "enter reject_invoice", {
            "invoiceId": str(invoice_id),
            "reviewerId": str(current_user.id) if current_user else None
        })
        invoice = session.get(Invoice, invoice_id)
        ...
        _dbg_log("pre-run", "H6", "invoice.py:reject_invoice", "exception", {
            "invoiceId": str(invoice_id),
            "error": str(e)
        })
        raise HTTPException(status_code=500, detail=f"审核失败: {str(e)}")
