...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    const handleOpenTab = (event: CustomEvent) => {
      ...
      console.log('Dashboard: openTab事件触发:', { type, data: data ?? undefined, fullEvent: event.detail })
      ...
      else if (type === "schema-config-list-page") {
        addTab({
          id: "schema-config-list-page",
          title: "Schema配置列表",
          content: <SchemaConfigList />,
          closable: true
        })
      }
      else if (type === "schema-list") {
        console.log('Dashboard: 处理schema-list类型，创建TAB页')
        addTab({
          id: "schema-list",
          title: "Schema配置列表",
          content: <SchemaConfigList />,
      ...
      else if (type === "schema-create") {
        addTab({
          id: "schema-create",
          title: "创建Schema",
          content: <SchemaConfigEdit isEdit={false} />,
          closable: true
        })
      }
      else if (type === "schema-edit") {
        const schemaData = event.detail?.data || null
        const title = schemaData?.name ? `编辑Schema: ${schemaData.name}` : "编辑Schema"
        addTab({
          id: `schema-edit-${schemaData?.id || 'unknown'}`,
          title: title,
          content: <SchemaConfigEdit schemaData={schemaData} isEdit={true} />,
          closable: true
        })
      }
